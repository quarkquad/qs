...
void HilbertTransform::calculateCoefficients(float sampleRate) {
    ...
    const float bandRatio = bandLow / bandHigh;
    const float pi = float(M_PI);

    const float k = sqrtf(1.0f - powf(bandRatio, 2.0f));
    const float l = 0.5f * ((1.0f - sqrtf(k)) / (1.0f + sqrtf(k)));
    const float m = l + (2.0f * (pow(l, 5.0f))) + (15.0f * (powf(l, 9.0f)));
    const float q = expf((powf(pi, 2.0f)) / logf(m));

    const int n = int(ceil(logf(0.2f * pi / 720.0f) / logf(q)));
    poles = n % 2 == 0 ? n : n + 1;
    coeffs.calloc(poles);
    histX.calloc(poles);
    histY.calloc(poles);
    ...
    auto calcPole = [&](float x) {
        const float y = atanf(((powf(q, 2.0f) - powf(q, 6.0f)) * sinf(4.0f * x)) /
                              (1.0f + (powf(q, 2.0f) + powf(q, 6.0f)) * cosf(4.0f * x)));
        return tanf(x - y);
    };
    auto calcCoeff = [&](float p) {
        const float g = (bandLow * pi * p) / sampleRate;
        return (g - 1.0f) / (g + 1.0f);
    };

    const float factor = pi / 4.0f / static_cast<float>(poles);
